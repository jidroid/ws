...
flowchart TB
    ...
    subgraph PG[Point read path]
      ...
      M1 -->|Not found| M2{Check immutable memtables, newest to oldest}

      M2 -->|Visible value| R2([Return value])
      M2 -->|Visible tombstone| RNF2([Key not found])
      M2 -->|Not found| L0F{Filter L0 SSTables by key range and bloom filter}

      L0F -->|May exist| L0I[Probe each matching L0 SSTable, newest to oldest]
      L0F -->|No match| LN

      L0I --> IDX0[Lookup key in SSTable index]
      IDX0 --> BLK0[Read data block cache if present else from disk]
      BLK0 --> HIT0{Key found?}
      ...
      LN --> Li[For each level, choose at most one SSTable by key range]
      Li --> BF[Check SSTable Bloom filter]
      BF -->|Definitely not| NEXT[Next level]
      BF -->|May exist| IDX[Binary search for key on SSTable index]

      IDX --> BLK[Read data block cache if present else from disk]
      BLK --> HIT{Key found?}
    ...
    subgraph RS[Range scan path]
      ...
      S --> I4[Create iterators for Levels 1..N overlapping the range]
